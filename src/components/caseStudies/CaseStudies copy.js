...
const CaseStudies = ({ caseStudiesData, ClrStandard }) => {
  ...
  const ImagesCardGrid = () => {
    return (
      ...
      className="grid grid-cols-1 md:grid-cols-2 gap-6 mx-auto md:mx-10"
      >
        {caseStudiesData[0].caseStudiesImageCard.map((v, i) => (
          <div
            key={i}
          ...
          >
            ...
            className="grid grid-rows-2 items-center mx-3 absolute top-0 sm:-top-5 md:top-10 text-white w-80 md:w-full"
            >
              {/* <h1 className="font-bold text-[15px] md:text-[18px] m:text-2xl"> */}
              <h1 className="font-bold text-xl md:text-[18px] m:text-2xl">
                {v.caseStudiesCardTitle}
              </h1>
              {/* <p className="my-2 text-[12px] md:text-[20px] sm:text-[14px]"> */}
              <p className="my-2 md:text-[20px] text-[20px]">
                {v.caseStudiesCardText2}
              </p>
              {v.caseStudiesCardBtn && (
                <button
  ...
  // const TabBar = () => {
  //   return (
  //     <div className="overflow-x-scroll bg-zinc-800 space-x-7 p-2 rounded-full text-white">
  //       {caseStudiesData[0].caseStudiesNavbar.map((tab, idx) => (
  //         <span className="hover:bg-green-600 hover:p-3 hover:rounded-full text-xs">
  //           <button>{tab.useCaseNavbarText}</button>
  //         </span>
  //       ))}
  //     </div>
  //   )
  ...
  // const TabBar = () => {
  //   return (
  //     <div className="overflow-x-scroll bg-zinc-800 mx-auto space-x-7 p-2 rounded-full text-white">
  //       {caseStudiesData[0].caseStudiesNavbar.map((tab, idx) => (
  //         <span
  //           className="hover:bg-green-600 hover:p-3 hover:rounded-full text-xs"
  //         >
  //           <button>{tab.useCaseNavbarText}</button>
  //         </span>
  //       ))}
  ...
  return (
    ...
    >
      ...
      >
        {/* <TabBar /> */}
        {/* <div className="bg-zinc-800 space-x-7 p-2 text-white w-full overflow-x-scroll"> */}
      ...
      <button
        style={rootStyle}
        className="bgClr w-[38%] md:w-[20%] my-5 p-2 rounded-full text-white font-bold mx-auto"
      >
